...
extract "$MY_DIR/mk-proprietary-blobs.txt" "$SRC"
...
if [ "$SRC" != "adb" ]; then
  for file in $(egrep -v '(^#|^$)' "$MY_DIR"/mk-proprietary-blobs-vendorimg.txt); do
...
extract "$MY_DIR/mk-proprietary-blobs-vendorimg.txt" "$SRC"
